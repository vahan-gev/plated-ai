...
export async function POST(request) {
    try {
        ...
        if (
            ...
            process.env.GEMINI_API_KEY === "REPLACE_ME_WITH_YOUR_GEMINI_API_KEY"
        ) {
        ...
        if (group.surfaceImage) {
            const surfacePath = `/assets/tablecloths/${group.surfaceImage}.png`;
            try {
        ...
        if (dish.hasCutlery && dish.cutleryStyleImage) {
            const cutleryPath = `/assets/cutleries/${dish.cutleryStyleImage}.png`;
            try {
...
function getAssetPath(type, vesselType, value) {
    if (type === "vessel") {
        if (value.startsWith("plate")) return `/assets/plates/${value}.png`;
        if (value.startsWith("bowl")) return `/assets/plates/${value}.png`;
        if (value.startsWith("board")) return `/assets/plates/${value}.png`;
        if (value.startsWith("cup") || value.startsWith("glass"))
            return `/assets/cups/${value}.png`;
    }
    return null;
}
